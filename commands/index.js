...
// ════════════════════════════════════════════════════════════
// ⚠️ لا تضع بيانات حقيقية هنا أبدًا — اعتمد فقط على متغيرات البيئة (.env / Render)
...
// ─── منع EPIPE وأخطاء الشبكة الصغيرة من إسقاط البوت، وإيقاف العملية
//     بأمان لأي استثناء آخر غير متوقَّع (الحالة بعد uncaughtException
//     قد تكون غير مستقرة حسب توثيق Node.js — الإيقاف هنا أكثر أمانًا
//     من الاستمرار، بافتراض أن Render يعيد تشغيل العملية تلقائيًا) ──
...
// ─── كائن message موحَّد (reply/unsend/registerReply) ─────────
// يُستخدَم في: ctx الأوامر، handler الردود المحفوظة، و onChat
// (كان مكرَّرًا حرفيًا في 3 مواضع قبل هذا الإصلاح)
    ...
    try {
      ...
      if (mod.config?.name && (mod.onStart || mod.run || mod.execute)) {
        ...
        if (global.commands.has(name)) {
          console.warn(`[WARN] تعارض اسم أمر: "${name}" من '${file}' يطغى على أمر سابق بنفس الاسم/alias`);
        }
        ...
        (mod.config.aliases || []).forEach(a => {
          ...
          if (global.commands.has(alias)) {
            console.warn(`[WARN] تعارض alias: "${alias}" من '${file}' يطغى على أمر/alias سابق بنفس الاسم`);
          }
...
const handleMessage = async (api, event) => {
  ...
  if (messageReply && global.Kagenou.replies?.[messageReply.messageID]) {
    ...
    if (!replyData.author || replyData.author === senderID) {
      // ← فحص الصلاحية وفترة الانتظار حتى للردود المحفوظة، تمامًا كأي
      //   أمر عادي — كان هذا غائبًا تمامًا من قبل (ثغرة على أوامر إدارية)
      const replyRole = replyData.role ?? 0;
      if (global.getUserRole(senderID) < replyRole) {
        return api.sendMessage("⚠️ هذا الإجراء للمشرفين فقط", threadID, null, messageID);
      }
      if (replyData.cooldownKey) {
        const cdMsg = global.checkCooldown(senderID, replyData.cooldownKey);
  ...
  try {
    ...
    // رسالة عامة للمستخدم فقط — التفاصيل الكاملة (مسارات/stack) تبقى في console
    console.error(`[CMD ERR] ${commandName}:`, err);
    api.sendMessage(`❌ حدث خطأ أثناء تنفيذ الأمر، حاول مرة أخرى لاحقًا.`, threadID, null, messageID);
  }
};
...
const handleEvent = async (api, event) => {
  ...
  // ← مقارنة بالاسم (config.name/aliases) بدل مرجع الكائن (===)؛ المرجع
  //   قد يختلف لو أُعيد تحميل الأمر (.up) بين بناء eventCommands ومعالجة
  //   الرسالة، فيفشل الفحص صامتًا ويؤدي لتنفيذ مزدوج (حالة نادرة سابقًا)
  function matchesFirstWord(cmd) {
    if (!firstWord || !cmd.config?.name) return false;
    if (cmd.config.name.toLowerCase() === firstWord) return true;
    return (cmd.config.aliases || []).some(a => a.toLowerCase() === firstWord);
  }
  ...
  for (const cmd of global.eventCommands) {
    try {
      if (cmd.onChat) {
        ...
        if (matchesFirstWord(cmd)) continue;
      ...
      console.error(`[onChat ERR] ${cmd.config?.name || "?"}:`, e);
    }
  }
};
...
function startWebServer() {
  ...
  (() => {
    ...
    // ─── حماية مسارات /yt/* — كانت مفتوحة بالكامل بلا أي تحقق هوية ─
    // يُسمح فقط بالاستدعاء الداخلي من localhost (مثل yt.js عبر نفس
    // العملية) أو لمن يحمل هيدر X-Internal-Key مطابق لـ INTERNAL_API_KEY
    function requireInternal(req, res, next) {
      const ip = req.ip || req.socket?.remoteAddress || "";
      const isLocal = ip === "127.0.0.1" || ip === "::1" || ip === "::ffff:127.0.0.1";
      const key = req.get("X-Internal-Key");
      const expectedKey = process.env.INTERNAL_API_KEY;
      if (isLocal || (expectedKey && key === expectedKey)) return next();
      return res.status(403).json({ error: "غير مسموح — هذا المسار داخلي فقط" });
    }
    ...
    // POST /yt/search
    app.post("/yt/search", requireInternal, async (req, res) => {
      try {
        const query = (req.body?.query || "").trim().slice(0, 200); // ← حد أقصى لطول النص
    ...
    // POST /yt/audio → MP3
    app.post("/yt/audio", requireInternal, async (req, res) => {
    ...
    // POST /yt/video → MP4
    app.post("/yt/video", requireInternal, async (req, res) => {
      const url = (req.body?.url || "").trim();
      if (!url) return res.status(400).json({ error: "url مطلوب" });
      let tmpPath = null;
      try {
...
function fallbackToEmailLogin(reason) {
  ...
  if (!FB_EMAIL || !FB_PASSWORD) {
    console.error(chalk.red("[LOGIN] ❌ بيانات الدخول (FB_EMAIL/FB_PASSWORD) غير مضبوطة في .env"));
    process.exit(1);
  }
